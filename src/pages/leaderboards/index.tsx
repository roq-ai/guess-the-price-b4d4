...
type ColumnType = ColumnDef<LeaderboardInterface, unknown>;

interface LeaderboardListPageProps {
...
export function LeaderboardListPage(props: LeaderboardListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<LeaderboardInterface>>(
    () => `/leaderboards?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: LeaderboardInterface) => {
    if (hasAccess('leaderboard', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/leaderboards/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/leaderboards/view/${record.id}`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              mr={2}
              padding="0rem 8px"
